import React from 'react'
import { Grid } from '@toptal/picasso'
import { Modal } from '@staff-portal/modals-service'
import { Form } from '@toptal/picasso-forms'
import { useNotifications } from '@toptal/picasso/utils'
import { useHandleMutationResult } from '@staff-portal/mutation-result-handlers'
import {
  AppointCommunityLeaderInput,
  CommunityLeaderStatus,
  CommunityLeaderType
} from '@staff-portal/graphql/staff'
import { RefetchQueries } from '@staff-portal/data-layer-service'
import { FormCancelButton } from '@staff-portal/forms'

import { useAppointCommunityLeader } from '../../data/appoint-community-leader'
import { CommunityLeaderBasicInfo } from '../../../../types'

interface Props {
  hideModal: () => void
  communityLeaderBasicInfo: CommunityLeaderBasicInfo
  refetchQueries?: RefetchQueries
}

const MakeCommunityLeaderModal = ({
  hideModal,
  communityLeaderBasicInfo,
  refetchQueries
}: Props) => {
  const { showError } = useNotifications()
  const { handleMutationResult } = useHandleMutationResult()

  const [appointCommunityLeader, { loading }] = useAppointCommunityLeader({
    onError() {
      showError('Unable to create community leader')
    }
  })

  const initialValues = {
    type: CommunityLeaderType.COMMUNITY_LEADER,
    memos: '',
    status: CommunityLeaderStatus.APPROVED
  }

  const handleSubmit = async (
    fields: Omit<AppointCommunityLeaderInput, 'id'>
  ) => {
    const { type = CommunityLeaderType.COMMUNITY_LEADER, memos = '' } = fields

    const { data } = await appointCommunityLeader({
      variables: {
        input: {
          type,
          memos,
          roleId: communityLeaderBasicInfo?.id as string
        }
      },
      refetchQueries
    })

    return handleMutationResult({
      mutationResult: data?.appointCommunityLeader,
      successNotificationMessage: `Community Leader ${communityLeaderBasicInfo?.fullName} was created successfully`,
      onSuccessAction: hideModal
    })
  }

  return (
    <Modal open onClose={hideModal} onBackdropClick={hideModal}>
      <Modal.Title>{`Assign ${communityLeaderBasicInfo?.fullName} as a Community Leader`}</Modal.Title>
      <Form onSubmit={handleSubmit} initialValues={initialValues}>
        <Modal.Content>
          <Grid spacing={16}>
            <Grid.Item small={12} medium={6}>
              <Form.RadioGroup
                name='type'
                label='Select the type of Community Leader'
                titleCase={false}
              >
                <Form.Radio
                  value={CommunityLeaderType.COMMUNITY_LEADER}
                  label='Full Community Leader'
                  data-testid='type-full-field'
                />
                <Form.Radio
                  value={CommunityLeaderType.ONLINE_LEADER}
                  label='Online Community Leader'
                  data-testid='type-online-field'
                />
              </Form.RadioGroup>
            </Grid.Item>
            <Grid.Item small={12}>
              <Form.Input
                name='memos'
                multiline
                rows={6}
                width='full'
                placeholder='Add a comment...'
                data-testid='memos-field'
              />
            </Grid.Item>
          </Grid>
        </Modal.Content>

        <Modal.Actions>
          <FormCancelButton onClick={hideModal} />
          <Form.SubmitButton loading={loading} variant='positive'>
            Confirm
          </Form.SubmitButton>
        </Modal.Actions>
      </Form>
    </Modal>
  )
}

export default MakeCommunityLeaderModal
