...
import EntityPerformedActionsListPage from './EntityPerformedActionsListPage'
import {
...
jest.mock('./EntityPerformedActionsList', () => ({
  __esModule: true,
...
const arrangeTest = ({
  ...
  render(
    <TestWrapper>
      <EntityPerformedActionsListPage />
    </TestWrapper>
  )
}

describe('EntityPerformedActionsListPage', () => {
  const entityId = '123'

  describe('when `entityType` is valid and supported', () => {
    const entityType = 'talents'

    it('renders performed actions list', () => {
      arrangeTest({ entityId, entityType })
