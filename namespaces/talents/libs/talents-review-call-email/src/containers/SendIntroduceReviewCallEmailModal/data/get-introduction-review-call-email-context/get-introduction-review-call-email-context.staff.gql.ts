...
import {
  GetIntroductionReviewCallEmailContextDocument,
  GetIntroductionReviewCallEmailContextQuery
} from './get-introduction-review-call-email-context.staff.gql.types'
import { EMAIL_MESSAGING_INTRODUCTION_REVIEW_CALL_FRAGMENT } from '../email-messaging-introduction-review-call-fragment/email-messaging-introduction-review-call-fragment.staff.gql'
...
export default gql`
  query GetIntroductionReviewCallEmailContext($nodeId: ID!) {
    node(id: $nodeId) {
      ... on Talent {
        id
        ...
        activation {
          ...
          steps {
            nodes {
              ...EmailMessagingIntroductionReviewCallFragment
            }
          }
        }
      }
  ...
  ${EMAIL_MESSAGING_INTRODUCTION_REVIEW_CALL_FRAGMENT}
`

const convertToEmailContext = (
  node: NonNullable<GetIntroductionReviewCallEmailContextQuery['node']>
  ...
  return convertToReviewCallEmailContext(
    activationStep?.emailMessagingIntroduction,
    contacts
  )
}

export const useGetIntroductionReviewCallEmailContext = ({
  nodeId,
  onCompleted
}: {
  nodeId: string
