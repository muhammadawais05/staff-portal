...
const arrangeTest = ({
  ...
  return render(
    <TestWrapper>
      <RestoreExpiredEngagementModal engagementId='123' hideModal={hideModal} />
    </TestWrapper>
  )
}

describe('RestoreExpiredEngagementModal', () => {
  it('renders and shows the success message after submit', async () => {
    const hideModal = jest.fn()

    arrangeTest({ hideModal })

    expect(screen.getByText('Restore Expired Interview')).toBeInTheDocument()
