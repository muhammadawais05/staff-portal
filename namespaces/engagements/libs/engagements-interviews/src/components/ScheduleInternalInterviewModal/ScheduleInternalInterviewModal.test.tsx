...
jest.mock(
  ...
  () => ({
    __esModule: true,
    default: () => (
      <div data-testid='schedule-internal-interview-modal-content' />
    )
  })
)
...
const renderComponent = () =>
  render(
    <TestWrapper>
      <ScheduleInternalInterviewModal engagementId='123' hideModal={() => {}} />
    </TestWrapper>
  )

describe('ScheduleInternalInterviewModal', () => {
  describe('when loading', () => {
    it('shows the modal suspender', () => {
      ...
      expect(
        screen.queryByTestId('schedule-internal-interview-modal-content')
  ...
  describe('when operation is enabled', () => {
    it('shows modal content', async () => {
      ...
      expect(screen.queryByTestId('modal-suspender')).not.toBeInTheDocument()
      expect(
        await screen.findByTestId('schedule-internal-interview-modal-content')
      ).toBeInTheDocument()
    })
  })
})
