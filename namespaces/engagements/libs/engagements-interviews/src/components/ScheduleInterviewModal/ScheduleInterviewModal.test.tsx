...
jest.mock(
  ...
  () => ({
    __esModule: true,
    default: () => <div data-testid='schedule-interview-modal-content' />
  })
)
...
const renderComponent = () =>
  render(
    <TestWrapper>
      <ScheduleInterviewModal engagementId='123' hideModal={() => {}} />
    </TestWrapper>
  )
...
describe('ScheduleInterviewModal', () => {
  describe('when loading', () => {
    it('shows the modal suspender', () => {
      ...
      expect(
        screen.queryByTestId('schedule-interview-modal-content')
  ...
  describe('when operation is enabled', () => {
    it('shows modal content', async () => {
      ...
      expect(screen.queryByTestId('modal-suspender')).not.toBeInTheDocument()
      expect(
        await screen.findByTestId('schedule-interview-modal-content')
      ).toBeInTheDocument()
    })
  })
})
